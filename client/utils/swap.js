...
*/
export const getAmountOfTokenReceivedFromSwap = async (_swapAmountWei, provider, ethSelected, ethBalance, reservedCD) => {
    ...
    if (ethSelected)
        amountOfTokens = await contract.getAmountOfTokens(
            _swapAmountWei,
            ethBalance,
            reservedCD
        );
    else amountOfTokens = await contract.getAmountOfTokens(
        _swapAmountWei,
        reservedCD,
        ethBalance
    )
...
export const swapTokens = async (signer, swapAmountWei, tokenToBeReceivedAfterSwap, ethSelected) => {
    // Create a new instance of the exchange and token contracts
    const exchangeContract = new Contract(EXCHANGE_CONTRACT_ADDRESS, EXCHANGE_ABI, signer)
